...
export function useTimeEntriesQuery() {
    ...
    const useTimeEntriesListQuery = (optionsRef) => {
        return useQuery({
            ...
            queryFn: async () => {
                ...
                if (optionsRef.status) params.append('status', optionsRef.status)
                if (optionsRef.billable) params.append('billable', optionsRef.billable)
                if (optionsRef.date_from) params.append('date_from', optionsRef.date_from)
                if (optionsRef.date_to) params.append('date_to', optionsRef.date_to)
    ...
    const useApprovalStatsQuery = () => {
        return useQuery({
            ...
            queryFn: async () => {
                const response = await axios.get('/api/time-entries/stats/approvals')
                return response.data.data
            },
    ...
    const useTimeEntryQuery = (timeEntryId) => {
        return useQuery({
            ...
            queryFn: async () => {
                const response = await axios.get(`/api/time-entries/${timeEntryId}`)
                return response.data.data
            },
            enabled: !!timeEntryId,
    ...
    const useTicketTimeEntriesQuery = (ticketId) => {
        return useQuery({
            ...
            queryFn: async () => {
                const response = await axios.get(`/api/tickets/${ticketId}/time-entries`)
                return response.data.data
            },
            enabled: !!ticketId,
            staleTime: 1000 * 60 * 2   // 2 minutes
        ...
        onSuccess: (data) => {
            ...
            if (data.data?.ticket_id) {
                queryClient.invalidateQueries({ 
                    queryKey: queryKeys.timeEntries.byTicket(data.data.ticket_id) 
                })
            }
        },
        onError: (error) => {
    ...
    const updateTimeEntryMutation = useMutation({
        mutationFn: async ({ id, ...timeEntryData }) => {
            const response = await axios.put(`/api/time-entries/${id}`, timeEntryData)
            return response.data
        },
        onMutate: async ({ id, ...timeEntryData }) => {
            // Cancel any outgoing refetches
            await queryClient.cancelQueries({ queryKey: queryKeys.timeEntries.byId(id) })
            
            // Snapshot the previous value
            const previousTimeEntry = queryClient.getQueryData(queryKeys.timeEntries.byId(id))
        ...
        onError: (err, { id }, context) => {
            ...
            if (context?.previousTimeEntry) {
                queryClient.setQueryData(queryKeys.timeEntries.byId(id), context.previousTimeEntry)
            }
            console.error('Failed to update time entry:', err)
        },
        onSettled: (data, error, { id }) => {
            // Refetch to ensure we have the latest data
            queryClient.invalidateQueries({ queryKey: queryKeys.timeEntries.byId(id) })
            queryClient.invalidateQueries({ queryKey: queryKeys.timeEntries.all })
            queryClient.invalidateQueries({ queryKey: queryKeys.timeEntries.stats() })
        }
    })
    ...
    const deleteTimeEntryMutation = useMutation({
        mutationFn: async (timeEntryId) => {
            const response = await axios.delete(`/api/time-entries/${timeEntryId}`)
            return response.data
        },
        onSuccess: (data, timeEntryId) => {
            // Remove from cache and invalidate related queries
            queryClient.removeQueries({ queryKey: queryKeys.timeEntries.byId(timeEntryId) })
            queryClient.invalidateQueries({ queryKey: queryKeys.timeEntries.all })
            queryClient.invalidateQueries({ queryKey: queryKeys.timeEntries.stats() })
        },
        onError: (error) => {
    ...
    const approveTimeEntryMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({ queryKey: queryKeys.timeEntries.all })
            queryClient.invalidateQueries({ queryKey: queryKeys.timeEntries.stats() })
            queryClient.invalidateQueries({ queryKey: queryKeys.timeEntries.approvalStats() })
        }
    })

    const rejectTimeEntryMutation = useMutation({
        mutationFn: async ({ timeEntryId, reason }) => {
            const response = await axios.post(`/api/time-entries/${timeEntryId}/reject`, { reason })
            return response.data
        },
        onMutate: async ({ timeEntryId }) => {
            // Optimistically update the time entry status
    ...
    const bulkApproveTimeEntriesMutation = useMutation({
        mutationFn: async ({ time_entry_ids, notes }) => {
            const response = await axios.post('/api/time-entries/bulk/approve', {
                time_entry_ids,
                notes
            })
            return response.data
