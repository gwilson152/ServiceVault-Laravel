<template>
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
        ...
        <div class="p-5">
          <div class="flex items-center">
            ...
            <div class="ml-5 w-0 flex-1">
              <dl>
                <dt class="text-sm font-medium text-gray-500 truncate">Total Hours (30d)</dt>
                <dd class="text-lg font-medium text-gray-900">{{ formatHours(stats.total_hours_month) }}</dd>
              </dl>
            </div>
          </div>
        </div>
      ...
      <div class="bg-white overflow-hidden shadow rounded-lg">
        <div class="p-5">
          <div class="flex items-center">
            ...
            <div class="ml-5 w-0 flex-1">
              <dl>
                <dt class="text-sm font-medium text-gray-500 truncate">Entries (30d)</dt>
                <dd class="text-lg font-medium text-gray-900">{{ stats.entries_count_month }}</dd>
              </dl>
            </div>
          </div>
        </div>
          ...
          <div class="flex items-center">
            ...
            <div class="ml-5 w-0 flex-1">
              <dl>
                ...
                <dd class="text-lg font-medium text-gray-900">{{ stats.pending_approval_count }}</dd>
              </dl>
            </div>
          </div>
        </div>
      ...
      <div class="bg-white overflow-hidden shadow rounded-lg">
        <div class="p-5">
          <div class="flex items-center">
            ...
            <div class="ml-5 w-0 flex-1">
              <dl>
                <dt class="text-sm font-medium text-gray-500 truncate">Avg/Entry</dt>
                <dd class="text-lg font-medium text-gray-900">
                  {{ formatHours(stats.entries_count_month > 0 ? stats.total_hours_month / stats.entries_count_month : 0) }}
                </dd>
              </dl>
            </div>
          </div>
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-md">
      <div class="px-4 py-5 sm:px-6 border-b border-gray-200">
        <h3 class="text-lg leading-6 font-medium text-gray-900">Time Entries</h3>
        <p class="mt-1 max-w-2xl text-sm text-gray-500">
          {{ timeEntries.total }} total entries
          <span v-if="timeEntries.total !== timeEntries.data.length">
      ...
      <ul v-else-if="timeEntries.data.length > 0" class="divide-y divide-gray-200">
        <li v-for="entry in timeEntries.data" :key="entry.id" class="px-4 py-4 sm:px-6 hover:bg-gray-50">
          <div class="flex items-center justify-between">
            <div class="flex items-center min-w-0">
              ...
              <div class="min-w-0 flex-1">
                ...
                <div class="mt-1 flex items-center text-sm text-gray-500">
                  <p>{{ entry.duration_formatted }} • {{ formatDate(entry.started_at) }}</p>
                  <span v-if="entry.account" class="mx-2">•</span>
                  <p v-if="entry.account">{{ entry.account.name }}</p>
                  <span v-if="entry.calculated_cost" class="mx-2">•</span>
                  <p v-if="entry.calculated_cost" class="font-medium">${{ entry.calculated_cost }}</p>
                </div>
              </div>
            </div>
      ...
      <div v-else class="text-center py-12">
        ...
        <p class="mt-1 text-sm text-gray-500">Get started by adding your first time entry.</p>
        <div class="mt-6">
          <button
            @click="showCreateModal = true"
            class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
      ...
      <!-- Pagination -->
      <div v-if="timeEntries.data.length > 0" class="bg-white px-4 py-3 border-t border-gray-200 sm:px-6">
        <div class="flex items-center justify-between">
          ...
          <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p class="text-sm text-gray-700">
                Showing
                <span class="font-medium">{{ timeEntries.from }}</span>
                to
                <span class="font-medium">{{ timeEntries.to }}</span>
                of
  ...
  <div v-if="showBulkApprovalModal" class="fixed inset-0 z-10 overflow-y-auto">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" @click="showBulkApprovalModal = false"></div>
      <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          <h3 class="text-lg leading-6 font-medium text-gray-900">Bulk Approve Time Entries</h3>
          <p class="mt-2 text-sm text-gray-500">
            Are you sure you want to approve {{ selectedEntries.length }} time entries?
          </p>
          <div class="mt-4">
