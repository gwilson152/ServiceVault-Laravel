  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-gray-600">Create reusable templates for common addon items that can be quickly added to tickets.</p>
      </div>
      <button
    ...
    <!-- Loading State -->
    <div v-if="addonTemplatesQuery.isLoading.value" class="flex justify-center py-8">
      <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-600"></div>
    </div>

    <!-- Error State -->
    <div v-else-if="addonTemplatesQuery.isError.value" class="text-center py-8">
      <p class="text-red-600">Failed to load addon templates</p>
      <button @click="addonTemplatesQuery.refetch()" class="mt-2 text-indigo-600 hover:text-indigo-500">
        Try again
      </button>
    </div>

    <!-- Templates List -->
    <div v-else-if="filteredTemplates.length > 0" class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <div
        ...
        <div class="flex items-start justify-between">
          <div class="flex-1">
            ...
            <div class="text-sm text-gray-500 space-y-1">
              <div v-if="template.sku" class="flex justify-between">
                <span>SKU:</span>
                <span class="font-medium">{{ template.sku }}</span>
              </div>
              <div class="flex justify-between">
                <span>Default Qty:</span>
                <span class="font-medium">{{ template.default_quantity }}</span>
              </div>
              <div class="flex justify-between">
                <span>Unit Price:</span>
                <span class="font-medium">${{ template.default_unit_price }}</span>
              </div>
              <div class="flex justify-between">
                <span>Billable:</span>
                <span :class="template.is_billable ? 'text-green-600' : 'text-gray-500'">
                  {{ template.is_billable ? 'Yes' : 'No' }}
                </span>
              </div>
            </div>
          </div>
    ...
    <div v-else class="text-center py-12 bg-white border border-gray-200 rounded-lg">
      ...
      <h3 class="mt-2 text-sm font-medium text-gray-900">
        {{ selectedCategory ? 'No templates in this category' : 'No addon templates' }}
      </h3>
      <p class="mt-1 text-sm text-gray-500">
        {{ selectedCategory ? 'Try selecting a different category.' : 'Get started by creating your first addon template.' }}
      </p>
      <div class="mt-6">
        <button
          @click="showCreateModal = true"
...
const filteredTemplates = computed(() => {
  ...
  return addonTemplates.value.filter(template => template.category === selectedCategory.value)
})
