...
const getItemTitle = (item) => {
  ...
  if (props.type === 'billing-rate') {
    ...
    // Add disambiguation for timer-specific rates
...
const getItemSubtitle = (item) => {
  if (props.type === 'billing-rate') {
    // Always show the rate, and include description if available
...
const initializeSelectedItem = () => {
  ...
  if (props.modelValue && props.items.length > 0) {
    ...
    if (props.type === 'billing-rate' && !item) {
      console.log('UnifiedSelector - Billing rate NOT FOUND:', {
        lookingFor: props.modelValue,
        availableRates: props.items.map(i => ({ id: getItemKey(i), name: i.name }))
      });
    }
    ...
    if (item) {
      selectedItem.value = item
      if (props.type === 'billing-rate') {
        console.log('UnifiedSelector - Billing rate selected successfully:', { id: getItemKey(item), name: item.name || item.title, rate: item.rate });
      }
    } else {
      selectedItem.value = null
      if (props.type === 'billing-rate') {
        // This should no longer happen with the timer-specific rate inclusion
