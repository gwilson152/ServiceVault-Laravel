<template>
  <div class="relative">
    ...
    <div v-if="selectedTicket" class="p-2 bg-blue-50 border border-blue-200 rounded-lg">
      <div class="flex items-center justify-between">
        <div>
          <p class="text-sm font-medium text-blue-900">#{{ selectedTicket.ticket_number }} - {{ selectedTicket.title }}</p>
          <div class="flex items-center space-x-2 mt-1">
            <span v-if="selectedTicket.status" class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium"
                  :class="getStatusClasses(selectedTicket.status)">
              {{ selectedTicket.status }}
            </span>
            <span v-if="selectedTicket.priority" class="text-xs text-blue-700">
    ...
    >
      ...
      <div v-else>
        ...
        >
          <div class="flex items-center justify-between">
            <div class="flex-1">
              <p class="text-sm font-medium text-gray-900">#{{ ticket.ticket_number }} - {{ ticket.title }}</p>
              <div class="flex items-center space-x-2 mt-1">
                <span v-if="ticket.status" class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium"
                      :class="getStatusClasses(ticket.status)">
                  {{ ticket.status }}
                </span>
                <span v-if="ticket.priority" class="text-xs text-gray-500">
                  {{ ticket.priority }} Priority
                </span>
                <span v-if="ticket.account" class="text-xs text-gray-500">
                  {{ ticket.account.name }}
                </span>
              </div>
            </div>
...
// Emits
const emit = defineEmits(['update:modelValue', 'ticket-selected'])
