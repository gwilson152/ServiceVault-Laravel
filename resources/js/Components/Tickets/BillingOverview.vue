    ...
    <div class="space-y-6">
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div v-else class="py-4 text-gray-500">
                <p class="text-sm">
                    Billing rates are determined automatically based on:
                </p>
                <ul
                    class="mt-2 text-sm list-disc list-inside space-y-1 text-gray-600"
                >
                ...
                <p class="mt-2 text-xs text-gray-500">
                    Time entries inherit rates when created.
                </p>
            </div>
        </div>
                        ...
                        >
                            ...
                            >
                                <Link
                                    ...
                                    :href="
                                        route('invoices.show', entry.invoice_id)
                                    "
            ...
            <div class="p-6 space-y-4">
                ...
                >
                    ...
                    <div class="text-right">
                        ...
                        <span
                            :class="getAddonStatusClasses(addon.status)"
...
const hasUnbilledItems = computed(() => {
    return (
        timeEntries.value.some((entry) => !entry.invoice_id) ||
        billableAddons.value.some(
            (addon) => addon.status === "approved" && !addon.invoice_id
        )
    );
});
...
const unbilledAddons = computed(() => {
    return billableAddons.value.filter(
        (addon) => addon.status === "approved" && !addon.invoice_id
    );
});
...
const loadBillingSummary = async () => {
    ...
    try {
        const response = await axios.get(
            `/api/tickets/${props.ticket.id}/billing-summary`
        );
...
const loadBillingRate = async () => {
    ...
    try {
        const response = await axios.get(
            `/api/tickets/${props.ticket.id}/billing-rate`
        );
...
const loadInvoices = async () => {
    ...
    try {
        const response = await axios.get(
            `/api/tickets/${props.ticket.id}/invoices`
        );
...
const loadBillableAddons = async () => {
    ...
    try {
        const response = await axios.get(
            `/api/tickets/${props.ticket.id}/addons?billable=true`
        );
...
const exportBillingReport = async () => {
    try {
        const response = await axios.get(
            `/api/tickets/${props.ticket.id}/billing-report`,
            {
                responseType: "blob",
            }
        );
        ...
        link.setAttribute(
            "download",
            `ticket-${props.ticket.ticket_number}-billing-report.pdf`
        );
