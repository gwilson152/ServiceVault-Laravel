    ...
    <div class="space-y-4">
        ...
        >
            ...
            <p class="text-sm mt-1">
                Activity will appear here as actions are taken on this ticket.
            </p>
        </div>
        ...
        <div v-else class="relative">
            ...
            <div class="space-y-4">
                ...
                >
                    ...
                    >
                        <div
                            :class="getActivityDotClasses(activity.type)"
                        ...
                        >
                            ...
                            <div class="flex items-center justify-between mb-2">
                                <div class="flex items-center space-x-2">
                                    <div
                                        class="w-5 h-5 bg-gray-300 rounded-full flex items-center justify-center flex-shrink-0"
                                    >
                                        <span
                            ...
                            <div v-if="activity.details">
                                ...
                                <div
                                    v-if="activity.type === 'status_change'"
                                ...
                                >
                                    ...
                                    <span
                                        v-if="activity.details.old_assignee"
                                        class="text-gray-600"
                                    >
                                        {{ activity.details.old_assignee }} →
                                    </span>
                                ...
                                >
                                    ...
                                    <div
                                        v-if="activity.details.description"
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <span
                                            ...
                                            >{{
                                                formatAddonType(
                                                    activity.details.addon_type
                                                )
                                            }}</span
                                        >
                                        <span
                                            v-if="
                                                activity.details.estimated_cost
                                            "
                                            class="text-green-600 text-xs"
                                        >
                                            Est: ${{
                                ...
                                <div
                                    ...
                                    "
                                    class="text-sm text-gray-700 bg-gray-50 rounded-md p-2 mt-1"
                                >
                                    <div
                                        v-html="
                                            formatMessage(
                                    ...
                                    ></div>
                                    <div
                                        v-if="activity.details.is_internal"
                                        class="mt-1"
                                    >
                                        <span
                                ...
                                >
                                    <div
                                        v-for="attachment in activity.details
                                            .attachments"
                                        :key="attachment.id"
                                        class="flex items-center space-x-2 text-sm"
                                    >
...
const formatActivityDescription = (activity) => {
    const descriptions = {
        ...
        status_change: `changed status from ${formatStatus(
            activity.details?.old_status
        )} to ${formatStatus(activity.details?.new_status)}`,
        assignment: activity.details?.old_assignee
            ? `reassigned ticket from ${activity.details.old_assignee} to ${
                  activity.details.new_assignee || "Unassigned"
              }`
            : `assigned ticket to ${
                  activity.details?.new_assignee || "Unassigned"
              }`,
        time_entry: `logged ${formatDuration(
            activity.details?.duration
        )} of work time`,
        addon: `${activity.details?.action || "created"} an add-on: ${
            activity.details?.title
        }`,
        attachment: `uploaded ${
            activity.details?.attachments?.length || 1
        } file(s)`,
...
const getActivityIcon = (type) => {
    const iconMap = {
        comment:
            "M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-3.582 8-8 8a8.959 8.959 0 01-4.906-1.524A11.956 11.956 0 012 21c0-1.68.322-3.283.913-4.755A12.026 12.026 0 012 12C2 7.582 5.582 4 10 4s8 3.582 8 8z",
        status_change:
            "M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15",
        assignment:
            "M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0z",
...
const loadActivity = async (resetPage = true) => {
    ...
    try {
        ...
        const response = await axios.get(
            `/api/tickets/${props.ticket.id}/activity?${params}`
        );
...
const loadStats = async () => {
    ...
    try {
        const response = await axios.get(
            `/api/tickets/${props.ticket.id}/activity-stats`
        );
