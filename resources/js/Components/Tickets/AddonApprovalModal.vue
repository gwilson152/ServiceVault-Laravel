  ...
  >
    <div class="space-y-4">
      ...
      <div class="bg-gray-50 rounded-lg p-4 space-y-2">
        <div v-if="addon?.category" class="flex justify-between text-sm">
          <span class="text-gray-600">Category:</span>
          <span class="font-medium text-gray-900">{{ formatCategory(addon?.category) }}</span>
        </div>
        <div class="flex justify-between text-sm">
          <span class="text-gray-600">Quantity:</span>
          <span class="font-medium text-gray-900">{{ addon?.quantity || 1 }}</span>
        </div>
        <div class="flex justify-between text-sm">
          <span class="text-gray-600">Unit Price:</span>
          <span class="font-medium text-green-600">${{ addon?.unit_price || 0 }}</span>
        </div>
        <div v-if="addon?.discount_amount > 0" class="flex justify-between text-sm">
          <span class="text-gray-600">Discount:</span>
          <span class="font-medium text-red-600">-${{ addon?.discount_amount }}</span>
        </div>
        ...
        <div class="flex justify-between text-sm">
          ...
          <span :class="addon?.is_billable ? 'text-green-600' : 'text-red-600'" class="font-medium">
...
const totalAmount = computed(() => {
  const subtotal = (props.addon?.quantity || 0) * (props.addon?.unit_price || 0)
  return Math.max(0, subtotal - (props.addon?.discount_amount || 0))
})

// Methods
const formatCategory = (category) => {
  return category?.charAt(0).toUpperCase() + category?.slice(1).replace('_', ' ') || 'Other'
}
