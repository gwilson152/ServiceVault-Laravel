    ...
    <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
      ...
      <div class="px-6 py-4 border-b border-gray-200">
        ...
        <p class="text-sm text-gray-600 mt-1">
          Create invoice for {{ ticket.title }} ({{ ticket.ticket_number }})
        </p>
      </div>
      ...
      <form @submit.prevent="submitForm" class="px-6 py-4 space-y-6">
        ...
        <div class="bg-gray-50 rounded-lg p-4">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div class="text-center">
              <div class="text-lg font-bold text-blue-600">{{ unbilledEntries.length }}</div>
              <div class="text-gray-600">Time Entries</div>
            </div>
            <div class="text-center">
              <div class="text-lg font-bold text-purple-600">{{ unbilledAddons.length }}</div>
              <div class="text-gray-600">Add-ons</div>
            </div>
            <div class="text-center">
              <div class="text-lg font-bold text-green-600">${{ totalAmount }}</div>
              <div class="text-gray-600">Total Amount</div>
            </div>
            <div class="text-center">
              <div class="text-lg font-bold text-orange-600">{{ totalHours }}h</div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            />
            <p v-if="errors.invoice_date" class="text-red-500 text-xs mt-1">{{ errors.invoice_date }}</p>
          </div>
        ...
        <div v-if="unbilledEntries.length > 0">
          <div class="flex items-center justify-between mb-3">
            <h4 class="text-sm font-medium text-gray-900">Time Entries</h4>
            <button
              type="button"
              @click="selectAllTimeEntries"
              class="text-xs text-blue-600 hover:text-blue-700"
            >
              {{ allTimeEntriesSelected ? 'Deselect All' : 'Select All' }}
            </button>
          </div>
          
          <div class="space-y-2 max-h-40 overflow-y-auto border border-gray-200 rounded-md">
            <div 
              ...
              <div class="flex items-center space-x-3">
                ...
                <div class="flex-1">
                  <div class="text-sm font-medium text-gray-900">
                    {{ entry.user?.name }} - {{ formatDate(entry.started_at) }}
                  </div>
                  <div class="text-xs text-gray-600">
                    {{ formatDuration(entry.duration) }} • {{ entry.description || 'No description' }}
                  </div>
                </div>
              </div>
        ...
        <div v-if="unbilledAddons.length > 0">
          <div class="flex items-center justify-between mb-3">
            <h4 class="text-sm font-medium text-gray-900">Add-ons</h4>
            <button
              type="button"
          ...
          <div class="space-y-2 max-h-40 overflow-y-auto border border-gray-200 rounded-md">
            ...
            >
              <div class="flex items-center space-x-3">
                ...
                />
                <div class="flex-1">
                  <div class="text-sm font-medium text-gray-900">{{ addon.title }}</div>
                  <div class="text-xs text-gray-600">
                    {{ formatAddonType(addon.type) }} • {{ addon.actual_hours || addon.estimated_hours }}h
                  </div>
                </div>
              </div>
              <div class="text-sm font-medium text-green-600">
        ...
        <div class="bg-blue-50 rounded-lg p-4">
          <h4 class="text-sm font-medium text-gray-900 mb-3">Invoice Totals</h4>
          <div class="space-y-2 text-sm">
            ...
            <div v-if="form.discount_amount > 0" class="flex justify-between">
              ...
              <span class="font-medium text-red-600">-${{ form.discount_amount.toFixed(2) }}</span>
            </div>
            ...
            <div class="flex justify-between text-lg font-bold border-t border-blue-200 pt-2">
              <span>Total:</span>
              <span class="text-green-600">${{ finalTotal }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-3">
          <div class="flex items-center">
            <input 
              ...
              class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
            />
            <label for="save_draft" class="ml-2 text-sm text-gray-700">
          ...
          <div v-if="!form.save_as_draft" class="flex items-center">
            <input 
              ...
              class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
            />
            <label for="auto_send" class="ml-2 text-sm text-gray-700">
      ...
      <div class="px-6 py-4 border-t border-gray-200 flex items-center justify-end space-x-3">
        ...
        <button
          @click="submitForm"
          :disabled="submitting || (!form.selected_time_entries.length && !form.selected_addons.length)"
          class="px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {{ submitting ? 'Generating...' : (form.save_as_draft ? 'Save Draft' : 'Generate Invoice') }}
        </button>
      </div>
    </div>
  </div>
...
const taxAmount = computed(() => {
  return ((subtotal.value - (form.value.discount_amount || 0)) * (form.value.tax_rate || 0) / 100).toFixed(2)
})

const finalTotal = computed(() => {
  return (subtotal.value - (form.value.discount_amount || 0) + parseFloat(taxAmount.value)).toFixed(2)
})
...
const selectAllTimeEntries = () => {
  if (allTimeEntriesSelected.value) {
    ...
    form.value.selected_time_entries = props.unbilledEntries.map(entry => entry.id)
  }
}
...
const validateForm = () => {
  ...
  if (form.value.selected_time_entries.length === 0 && form.value.selected_addons.length === 0) {
    errors.value.general = 'Please select at least one time entry or add-on to invoice'
  }
...
const submitForm = async () => {
  ...
  try {
    ...
    if (error.response?.data?.errors) {
      ...
      errors.value = { general: 'Failed to generate invoice. Please try again.' }
    }
  } finally {
