                    ...
                    <div class="flex">
                        ...
                        <div class="text-sm">
                            ...
                            <p class="text-yellow-600 mt-1">
                                Basic item details cannot be changed because
                                this add-on has been {{ addon?.status }}.
                            </p>
                        </div>
                    </div>
                </div>
...
const canEditBasicFields = computed(() => {
    return (
        props.addon?.status === "pending" || props.addon?.status === "rejected"
    );
});
...
const populateForm = () => {
    ...
    form.value = {
        ...
        billable:
            props.addon.billable !== undefined ? props.addon.billable : true,
        is_taxable:
            props.addon.is_taxable !== undefined
                ? props.addon.is_taxable
                : true,
...
const submitForm = async () => {
    ...
    try {
        ...
        if (error.response?.data?.errors) {
            ...
            errors.value = {
                general: "Failed to update add-on. Please try again.",
            };
        }
    } finally {
