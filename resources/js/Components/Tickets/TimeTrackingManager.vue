<template>
  <div class="space-y-6">
    ...
    <div v-if="activeTimers.length > 0">
      <h3 class="text-lg font-semibold text-gray-900 mb-4">Active Timers</h3>
      <div class="space-y-3">
        <div 
        ...
        >
          <div class="flex items-center space-x-3">
            ...
            <div>
              <p class="text-sm font-medium text-gray-900">{{ timer.user?.name || 'Unknown User' }}</p>
              <p v-if="timer.description" class="text-xs text-gray-600">{{ timer.description }}</p>
            </div>
          </div>
          <div class="flex items-center space-x-3">
            <span class="text-lg font-bold text-green-700">{{ formatDuration(timer.duration) }}</span>
            <div class="flex items-center space-x-2">
              <button
                v-if="timer.user_id === currentUserId && timer.status === 'running'"
              ...
              <button
                v-if="timer.user_id === currentUserId && timer.status === 'paused'"
              ...
              <button
                v-if="canManageAllTimers && timer.user_id !== currentUserId"
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-else v-for="entry in timeEntries" :key="entry.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm font-medium text-gray-900">{{ formatDuration(entry.duration) }}</div>
                <div v-if="entry.break_duration" class="text-xs text-gray-500">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span :class="getStatusClasses(entry.status)" class="px-2 py-1 rounded-full text-xs font-medium">
                  {{ formatStatus(entry.status) }}
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <span v-if="entry.billable" class="text-green-600 text-sm">
    ...
    <div class="bg-gray-50 rounded-lg p-6">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="text-center">
          <div class="text-2xl font-bold text-gray-900">{{ formatDuration(summary.total_time) }}</div>
          <div class="text-sm text-gray-600">Total Time</div>
        </div>
        <div class="text-center">
          <div class="text-2xl font-bold text-green-600">{{ formatDuration(summary.billable_time) }}</div>
          <div class="text-sm text-gray-600">Billable</div>
        </div>
        <div class="text-center">
          <div class="text-2xl font-bold text-blue-600">${{ summary.total_amount }}</div>
          <div class="text-sm text-gray-600">Total Value</div>
        </div>
        <div class="text-center">
          <div class="text-2xl font-bold text-purple-600">{{ summary.entries_count }}</div>
...
const canDeleteTimeEntry = (entry) => {
  return (entry.user_id === currentUserId.value && entry.status === 'pending') || canManageAllTimers.value
}
...
const loadActiveTimers = async () => {
  ...
  try {
    const response = await axios.get(`/api/tickets/${props.ticket.id}/timers`)
...
const loadSummary = async () => {
  ...
  try {
    const response = await axios.get(`/api/tickets/${props.ticket.id}/time-summary`)
