                ...
                <div v-if="showAssignmentSection" class="space-y-6">
                    ...
                    <div v-if="form.account_id && canAssignCustomers">
                        ...
                        <p class="mt-1 text-xs text-gray-500">
                            Select the customer user this ticket is for (if
                            applicable)
                        </p>
                    </div>
                    ...
                    <div v-if="canAssignTickets">
                        ...
                        />
                        <p class="mt-1 text-xs text-gray-500">
                            Select the agent to assign this ticket to (optional)
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-gray-50 rounded-lg p-4 space-y-4">
                    ...
                    <label class="flex items-center">
                        ...
                        <span class="ml-2 text-sm text-gray-700"
                            >Send email notifications to relevant users</span
                        >
                    </label>
                </div>
...
// Template refs
const accountSelectorRef = ref(null);
...
const canPresetCurrentUserAsAgent = computed(() => {
    ...
    // Check various agent permission conditions
    const hasTicketAgent = user.value.permissions?.includes('tickets.act_as_agent');
    const hasAdminWrite = user.value.permissions?.includes('admin.write');
    const isSuperAdmin = user.value.user_type === 'super_admin';
    const isAgent = user.value.user_type === 'agent';
...
const loadCustomers = async () => {
    ...
    try {
        ...
        const response = await axios.get(
            `/api/accounts/${form.account_id}/users`,
            {
                params: {
                    per_page: 100,
...
const submitForm = async () => {
    try {
        ...
        Object.assign(form, {
            ...
            customer_id: null,
            // Only preset agent if user can legitimately be assigned to this account
            agent_id: (props.preselectedAgentId && canPresetCurrentUserAsAgent.value) ? props.preselectedAgentId : null,
            priority: "normal",
            category: "",
            due_date: "",
            tags: "",
        ...
        if (error.response?.status === 422 && 
            ...
            form.agent_id = null;
            errors.value = {
                agent_id: 'Current user cannot be assigned to this account. Ticket created without agent assignment.'
            };
            
            // Retry submission without agent
            try {
                const retryPayload = { ...form };
                delete retryPayload.agent_id;
                
                // Process the payload the same way
                if (retryPayload.tags) {
                    retryPayload.tags = retryPayload.tags
                        .split(",")
                        .map((tag) => tag.trim())
                        .filter((tag) => tag);
                } else {
                ...
                if (retryPayload.due_date) {
                    retryPayload.due_date = new Date(retryPayload.due_date).toISOString();
                }
                if (!retryPayload.account_id) {
                    delete retryPayload.account_id;
                }
                if (!retryPayload.customer_id) {
