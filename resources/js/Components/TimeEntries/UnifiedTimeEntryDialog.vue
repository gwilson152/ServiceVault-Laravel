  ...
  <Modal :show="show" @close="closeDialog" max-width="3xl">
    <div class="p-6">
      ...
      <form @submit.prevent="submitTimeEntry" class="space-y-6">
        ...
        <div v-if="mode === 'timer-commit' && timerData" class="bg-gray-50 dark:bg-gray-600 rounded-md p-4">
          <div class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Timer Information</div>
          <div class="text-sm text-gray-900 dark:text-gray-100">{{ timerData.description || 'Timer' }}</div>
          <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">
            Duration: {{ formatDuration(calculateTimerDuration()) }}
            <span v-if="timerData.billing_rate_id && form.estimatedValue > 0" class="ml-2">
              Estimated Value: ${{ form.estimatedValue.toFixed(2) }}
            </span>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between pt-6 border-t border-gray-200">
          ...
          <div class="flex items-center space-x-3">
            <div v-if="totalBillingAmount > 0" class="text-sm text-gray-600">
...
const props = defineProps({
  ...
  mode: {
    ...
    validator: value => ['create', 'edit', 'timer-commit'].includes(value)
  },
  timerData: {
    type: Object,
    default: null
...
const { settings, validation, formatDuration, validateDuration, requiresApproval } = useTimerSettings()
...
const dialogTitle = computed(() => {
  switch (props.mode) {
    case 'create': return 'Add Time Entry'
    case 'edit': return 'Edit Time Entry'
    case 'timer-commit': return 'Commit Timer to Time Entry'
...
const dialogSubtitle = computed(() => {
  switch (props.mode) {
    case 'create': return 'Manually log time spent on this ticket'
    case 'edit': return 'Update the time entry details'
    case 'timer-commit': 
      return props.timerData 
        ? `Timer has been running for ${formatDuration(calculateTimerDuration())}` 
...
const submitButtonText = computed(() => {
  switch (props.mode) {
    case 'create': return 'Add Time Entry'
    case 'edit': return 'Update Time Entry'
    case 'timer-commit': return 'Commit Time Entry'
    default: return 'Save'
  }
})

const canAssignToOthers = computed(() => {
  return user.value?.permissions?.includes('time.admin') || 
...
const submitTimeEntry = async () => {
  ...
  try {
    ...
    if (props.mode === 'edit' && props.timeEntry) {
      response = await axios.put(`/api/time-entries/${props.timeEntry.id}`, payload)
    } else {
    ...
    if (error.response?.data?.errors) {
      ...
      errors.value.general = error.response?.data?.message || 'An error occurred while saving the time entry'
    }
  } finally {
...
watch(() => [form.value.durationHours, form.value.durationMinutes], () => {
  calculateBillingAmount()
})

watch(() => form.value.billable, () => {
  calculateBillingAmount()
})
